...
_global_env = dotenv_values(".env")
_openai_key = _global_env.get("OPENAI_API_KEY") or os.environ.get("OPENAI_API_KEY")

llm = ChatOpenAI(model="gpt-4.1-mini", api_key=_openai_key)
...
# ============================================================
# OPENAI IMAGE GENERATION (REPLACES GEMINI)
# ============================================================
...
def generate_and_place_images(state: State) -> dict:
    ...
    for spec in image_specs:
        ...
        if not out_path.exists():
            try:
                img_size = spec.get("size", "1024x1024")
                img_bytes = _openai_generate_image_bytes(spec["prompt"], size=img_size)
                out_path.write_bytes(img_bytes)

            except Exception as e:
                # graceful fallback (don’t break blog)
                prompt_block = (
